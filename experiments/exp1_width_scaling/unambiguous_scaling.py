...
alias-free schema up to N=3200, sweeping Random and SVD projections.
"""
...
def main():
    ...
    for N in N_values:
        ...
        for W in widths:
            ...
            rand_res = evaluate_projection(statements_emb, queries_emb, facts, "random", W=W, R=R)
            configs_evaluated.append({"type": "random", "W": W, "results": rand_res})
            
            # 3. SVD projection
            print(f"  [*] Evaluating SVD (W={W})...")
            V_W = Vh[:W]
            svd_res = evaluate_projection(statements_emb, queries_emb, facts, "svd", W=W, statements_mean=statements_mean, V_W=V_W)
