...
==============================================================
Two-stage retrieval pipeline that tests whether a re-ranker can break
through the Relational Aliasing ceiling discovered in Experiment 3.

Telemetry:
  - Overall / per-template / per-alias-cardinality recall
  - Oracle@K (perfect re-ranker ceiling)
  - Token-matching and Cross-Encoder re-ranking
  - Reranker Gain (Δ) vs. alias cardinality
"""
...
def evaluate_rerankers(facts, statements_emb, queries_emb, cross_encoder_model,
    ...
    # We'll batch all queries' candidate pairs together for efficiency
    ...
    for q_idx in range(N):
        ...
        for c_idx in candidates:
            # Use only the first exemplar (index 0) for max-score cross-encoder
            # to keep compute reasonable. We'll also do mean across all 10.
...
def main():
    parser = argparse.ArgumentParser(description="Experiment 4: Relational Re-Ranker Evaluation")
    parser.add_argument("--N_max", type=int, default=800, help="Maximum number of classes to evaluate")
    parser.add_argument("--K", type=int, default=10, help="Top-K candidates for re-ranking")
    ...
    # Determine N values to sweep
    N_values = [n for n in [100, 200, 400, 800] if n <= args.N_max]
    ...
    for N in N_values:
        ...
        print(f"  [*] Running re-ranking evaluation (K={args.K})...")
        results = evaluate_rerankers(facts, statements_emb, queries_emb,
                                      cross_encoder, K=args.K, device='cpu')
