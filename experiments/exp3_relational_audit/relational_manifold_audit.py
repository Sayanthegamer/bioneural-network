...
def run_metric_evaluation(statements, queries, labels, metric, k_list=[1, 5, 10, 20]):
    ...
    correct_proto_mask = (dists_proto.argmin(dim=1) == torch.arange(N, device=device))
    recall_proto = correct_proto_mask.sum().item() / float(N)
    
    # 2. 1-NN evaluation
    pred_1nn_all = dists_all.argmin(dim=1) // num_exemplars
    correct_1nn_mask = (pred_1nn_all == torch.arange(N, device=device))
    recall_1nn = correct_1nn_mask.sum().item() / float(N)
    
    proto_advantage = recall_proto - recall_1nn
    
    # Ranks
    # Prototype ranks
    sorted_p_classes = dists_proto.argsort(dim=1)
    proto_ranks = (sorted_p_classes == torch.arange(N, device=device).unsqueeze(1)).nonzero()[:, 1].tolist()
    
    # Exemplar ranks (min distance to class exemplars)
    class_min_dists_all = dists_all.view(N, N, num_exemplars).min(dim=2).values # [N, N]
    sorted_e_classes = class_min_dists_all.argsort(dim=1)
    exemplar_ranks = (sorted_e_classes == torch.arange(N, device=device).unsqueeze(1)).nonzero()[:, 1].tolist()
    
    # Top-K accuracies
    centroid_topk_accs = {k: (torch.tensor(proto_ranks) < k).sum().item() / float(N) for k in k_list}
    exemplar_topk_accs = {k: (torch.tensor(exemplar_ranks) < k).sum().item() / float(N) for k in k_list}
    ...
    # Prototype Margin
    p_dist_correct = dists_proto.diag()
    min_indices = dists_proto.argmin(dim=1)
    is_correct = (min_indices == torch.arange(N, device=device))
    second_min_indices = sorted_p_classes[:, 1]
    wrong_indices = torch.where(is_correct, second_min_indices, min_indices)
    p_dist_wrong = dists_proto[torch.arange(N, device=device), wrong_indices]
    margins = (p_dist_wrong - p_dist_correct).tolist()
    ...
    # Lcomp ratio
    lcomp_ratios = (dists_proto.diag() / (class_min_dists_all.diag() + 1e-8)).tolist()
    ...
    if metric == "L1":
        intra_dists = torch.norm(statements - centroids.unsqueeze(1), p=1, dim=2)
    elif metric == "L2":
        intra_dists = torch.norm(statements - centroids.unsqueeze(1), p=2, dim=2)
    elif metric == "Cosine":
        statements_norm = statements / (statements.norm(p=2, dim=2, keepdim=True) + 1e-8)
        centroids_norm = centroids / (centroids.norm(p=2, dim=1, keepdim=True) + 1e-8)
        intra_dists = 1.0 - (statements_norm * centroids_norm.unsqueeze(1)).sum(dim=2)
    
    intra_vals = intra_dists.flatten().tolist()
