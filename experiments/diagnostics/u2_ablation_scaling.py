...
def run_width_evaluation(width, num_facts, seeds, pipeline, facts, embedded_statements, embedded_queries, shift_w, scale_w):
    ...
    for seed in seeds:
        ...
        # u2_enabled=False to ablate the slow-timescale attractor cascade
        engine = MESUEngine(model, lr=1.0, sigma_prior=0.1, alpha_decay=0.02, u2_enabled=False)
...
def main():
    print("====================================================")
    print("  MESU u2-Ablation Scaling Experiment (Optimized)")
    print("====================================================")
    ...
    results_dir = "experiments/results/diagnostic_telemetry"
    os.makedirs(results_dir, exist_ok=True)
    csv_path = os.path.join(results_dir, "u2_ablation_scaling.csv")
        ...
        for n_facts in capacity_steps:
            ...
            with open(csv_path, 'a', newline='') as f:
                ...
                for r in run_results:
                    writer.writerow({
                        "seed": r["seed"],
                        "experiment": "u2_ablation_scaling",
    ...
    print("\n====================================================")
    print("  u2-Ablation Scaling Summary Table (Mean Recall)")
    print("====================================================")
    ...
    plt.title("Width vs Recall (u2 Ablated)")
    plt.xscale('log')
    ...
    plt.title("Log-Log Recall Scaling (u2 Ablated)")
    plt.grid(True, which="both", ls="-", alpha=0.5)
    plt.legend()
    
    plt.tight_layout()
    plot_path = os.path.join(results_dir, "u2_ablation_scaling_plots.png")
    plt.savefig(plot_path)
    print(f"\n[*] Plots saved to: {plot_path}")
    
    # Output markdown report file
    md_path = os.path.join(results_dir, "u2_ablation_scaling_summary.md")
    with open(md_path, 'w') as f:
        f.write("# MESU u2-Ablation Scaling Experiment Analysis\n\n")
        f.write("## Scaling Law Fitting Table (u2-Ablated)\n\n")
