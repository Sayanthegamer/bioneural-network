# ==============================================================================
# SELF-CONTAINED EXPERIMENT 5: UNAMBIGUOUS SCHEMA SCALING FOR KAGGLE
# ==============================================================================
...
# Experiment 5 scaling sweep up to N=3200 with Random and SVD projections.
...
def generate_report(results_by_N, md_path, capacity_breakpoints):
    ...
    lines = []
    lines.append("# Experiment 5: Unambiguous Schema Scaling & True Capacity Validation (Kaggle)\n")
...
def generate_plots(results_by_N, plot_path):
    ...
    fig.suptitle("Experiment 5: Scaling Metrics under Unambiguous Schema (Kaggle)", fontsize=14, fontweight='bold')
...
def main():
    ...
    for N in N_values:
        ...
        with torch.no_grad():
            statements_flat = bi_encoder.encode(all_statement_texts, batch_size=256, convert_to_tensor=True, device=device)
            statements_emb = statements_flat.view(N, 10, -1)
            queries_emb = bi_encoder.encode(all_query_texts, batch_size=256, convert_to_tensor=True, device=device)
            
        statements_flat_cpu = statements_flat.cpu()
        statements_emb_cpu = statements_emb.cpu()
        queries_emb_cpu = queries_emb.cpu()
        
        # Fit SVD projection matrix V_W for largest width (128)
        statements_mean = statements_flat_cpu.mean(dim=0, keepdim=True)
        statements_centered = statements_flat_cpu - statements_mean
